...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/xsakyx/RobloxUILib/main/RenLib.lua"))()
...
ensureManagedKeybindAPI(Library)

local AimbotToggleKeybind = Library:RegisterKeybind({Name = "Aimbot Toggle", Default = "F", Flag = "MasacreAimbotKey", Mode = "Press"})
local TargetModeKeybind = Library:RegisterKeybind({Name = "Target Everyone Toggle", Default = "B", Flag = "MasacreTargetModeKey", Mode = "Press"})
local LightHelmetKeybind = Library:RegisterKeybind({Name = "Light Helmet Toggle", Default = "H", Flag = "MasacreLightHelmetKey", Mode = "Press"})
local NoclipKeybind = Library:RegisterKeybind({Name = "Noclip Toggle", Default = "G", Flag = "MasacreNoclipKey", Mode = "Press"})
local SprintKeybind = Library:RegisterKeybind({Name = "Sprint", Default = "LeftShift", Flag = "MasacreSprintKey", Mode = "Hold"})
...
KeybindsSection:CreateLabel("Feature shortcuts are editable in the Keybind Manager")
KeybindsSection:CreateLabel("Managed: Aimbot, Target Everyone, Light Helmet, Noclip, Sprint")
KeybindsSection:CreateLabel("RMB (hold) = Aim at target")
KeybindsSection:CreateLabel("The UI visibility shortcut is managed there too")
...
InfoSection:CreateLabel("Noclip: Walk through walls; its shortcut is managed by RenLib")
InfoSection:CreateLabel("Sprint: Hold its managed PC shortcut or tap the mobile button for speed 20")
...
table.insert(Connections, UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if Library:IsCapturingKeybind() then return end
    if UserInputService:GetFocusedTextBox() then return end
    if input.UserInputType ~= Enum.UserInputType.Keyboard then return end

    if input.KeyCode.Name == AimbotToggleKeybind:Get() then
    ...
    if input.KeyCode.Name == TargetModeKeybind:Get() then
    ...
    if input.KeyCode.Name == LightHelmetKeybind:Get() then
    ...
    if input.KeyCode.Name == NoclipKeybind:Get() then
    ...
    if input.KeyCode.Name == SprintKeybind:Get() and SprintEnabled and not IsMobile then
        StartSprint()
    end
end))
...
table.insert(Connections, UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Keyboard
        and input.KeyCode.Name == SprintKeybind:Get()
        and SprintHeld
    then
        StopSprint()
    end
