print("QcuikDraw script loaded , made by xsakyx for RenCore .")
...
-- RenLib V7 multi-dropdowns expose a selected-value map. The game systems
-- intentionally keep their original array shape so existing table.find calls
-- and behavior remain unchanged.
local function selectedDropdownValues(selection)
    local values = {}
...
local AimbotToggleKeybind = RenLib:RegisterKeybind({Name = "Aimbot Toggle", Default = "V", Flag = "QuickdrawAimbotKey", Mode = "Press"})
local ESPToggleKeybind = RenLib:RegisterKeybind({Name = "ESP Toggle", Default = "B", Flag = "QuickdrawESPKey", Mode = "Press"})
local CrosshairToggleKeybind = RenLib:RegisterKeybind({Name = "Crosshair Toggle", Default = "N", Flag = "QuickdrawCrosshairKey", Mode = "Press"})
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if keyName == AimbotToggleKeybind:Get() then
        CombatSystem.Aimbot.Enabled = not CombatSystem.Aimbot.Enabled
    elseif keyName == ESPToggleKeybind:Get() then
        if CombatSystem.ESP.Enabled then
            ESPEngine.Disable()
        else
            ESPEngine.Enable()
        end
    elseif keyName == CrosshairToggleKeybind:Get() then
...
local AimbotTab = Window:CreateTab({Name = "Aimbot", Icon = 6031154871})

local AimbotSection1 = AimbotTab:CreateSection({Name = "Main Controls", Side = "Left"})

AimbotSection1:CreateToggle({
   Name = "Enable Aimbot",
   Default = false,
   Flag = "Aimbot",
...
local AimbotSection2 = AimbotTab:CreateSection({Name = "Settings", Side = "Right"})

AimbotSection2:CreateToggle({
...
AimbotSection2:CreateDropdown({
   ...
   Callback = function(Option)
      CombatSystem.Aimbot.TargetPart = Option
   end,
})

AimbotSection2:CreateSlider({
   Name = "FOV Size",
   Min = 50,
   Max = 500,
   Step = 10,
...
local AimbotSection3 = AimbotTab:CreateSection({Name = "Ignore List", Side = "Left"})

local IgnoreDropdown = AimbotSection3:CreateDropdown({
   ...
   Callback = function(Options)
      CombatSystem.Aimbot.IgnoredPlayers = selectedDropdownValues(Options)
   end,
})

AimbotSection3:CreateButton({
...
local CrosshairTab = Window:CreateTab({Name = "Crosshair", Icon = 6034328955})

local CrosshairSection1 = CrosshairTab:CreateSection({Name = "Crosshair Controls", Side = "Left"})

CrosshairSection1:CreateToggle({
...
CrosshairSection1:CreateLabel("Dynamic prediction shows where")
CrosshairSection1:CreateLabel("your bullet will land (yellow dot)")

local ESPTab = Window:CreateTab({Name = "ESP", Icon = 6034316009})

local ESPSection1 = ESPTab:CreateSection({Name = "ESP Controls", Side = "Left"})

ESPSection1:CreateToggle({
   Name = "Enable ESP",
   Default = false,
   Flag = "ESP",
   Callback = function(Value)
...
local ESPSection2 = ESPTab:CreateSection({Name = "Settings", Side = "Right"})

ESPSection2:CreateToggle({
   Name = "Show Names",
   Default = true,
   Flag = "Names",
   Callback = function(Value)
...
local BlacklistTab = Window:CreateTab({Name = "Blacklist", Icon = 6031094678})

local BlacklistSection1 = BlacklistTab:CreateSection({Name = "Blacklist System", Side = "Left"})

BlacklistSection1:CreateLabel("Blacklisted players show in")
BlacklistSection1:CreateLabel("opposite color with extra box")

BlacklistSection1:CreateToggle({
...
local BlacklistDropdown = BlacklistSection1:CreateDropdown({
   Name = "Select Players",
   Values = GetAllPlayers(),
   Default = {},
   Multi = true,
   ...
   Callback = function(Options)
      CombatSystem.Blacklist.Players = selectedDropdownValues(Options)
      ESPEngine.Refresh()
   end,
})

BlacklistSection1:CreateButton({
   Name = "Refresh List",
   Callback = function()
...
local BlacklistSection2 = BlacklistTab:CreateSection({Name = "Colors", Side = "Right"})

local ColorLabel1 = BlacklistSection2:CreateLabel("ESP: RGB(255, 0, 0)")
local ColorLabel2 = BlacklistSection2:CreateLabel("Blacklist: RGB(0, 255, 255)")

task.spawn(function()
    while task.wait(1) do
        SafeCall(function()
            local c = CombatConfig.ESPColor
            ColorLabel1:SetText("ESP: RGB(" .. math.floor(c.R*255) .. ", " .. math.floor(c.G*255) .. ", " .. math.floor(c.B*255) .. ")")
            
            local b = CombatSystem.Blacklist.Color
            ColorLabel2:SetText("Blacklist: RGB(" .. math.floor(b.R*255) .. ", " .. math.floor(b.G*255) .. ", " .. math.floor(b.B*255) .. ")")
        end)
    end
end)

local StatsTab = Window:CreateTab({Name = "Stats", Icon = 6034287594})

local StatsSection1 = StatsTab:CreateSection({Name = "Performance", Side = "Left"})

local FPSLabel = StatsSection1:CreateLabel("FPS: 0")
local FrameLabel = StatsSection1:CreateLabel("Frame Time: 0ms")

local StatsSection2 = StatsTab:CreateSection({Name = "Aimbot", Side = "Right"})

local TargetLabel = StatsSection2:CreateLabel("Target: None")
local DistLabel = StatsSection2:CreateLabel("Distance: 0m")

task.spawn(function()
    while task.wait(0.5) do
        SafeCall(function()
            FPSLabel:SetText("FPS: " .. PerformanceStats.FPS)
            FrameLabel:SetText("Frame Time: " .. string.format("%.1f", PerformanceStats.FrameTime) .. "ms")
            
            if CombatSystem.Aimbot.CurrentTarget then
                TargetLabel:SetText("Target: " .. CombatSystem.Aimbot.CurrentTarget.Name)
                DistLabel:SetText("Distance: " .. math.floor(GetDistance(CombatSystem.Aimbot.CurrentTarget)) .. "m")
            else
                TargetLabel:SetText("Target: None")
                DistLabel:SetText("Distance: 0m")
            end
        end)
    end
end)
...
local SettingsSection1 = SettingsTab:CreateSection({Name = "Information", Side = "Left"})

SettingsSection1:CreateLabel("FastCast Combat System")
SettingsSection1:CreateLabel("Optimized for realistic games")
SettingsSection1:CreateLabel("")
SettingsSection1:CreateLabel("Features:")
SettingsSection1:CreateLabel("✓ Fast Aimbot (No Priority)")
SettingsSection1:CreateLabel("✓ Only targets players in FOV")
SettingsSection1:CreateLabel("✓ Bullet drop compensation")
SettingsSection1:CreateLabel("✓ Dynamic crosshair prediction")
SettingsSection1:CreateLabel("✓ Box ESP with health bars")
SettingsSection1:CreateLabel("✓ Blacklist system")
SettingsSection1:CreateLabel("✓ 2000+ lines of code")

local SettingsSection2 = SettingsTab:CreateSection({Name = "Controls", Side = "Right"})

SettingsSection2:CreateButton({
   Name = "Disable All",
   Callback = function()
