...
local AimbotKeybind = Library:RegisterKeybind({
    Name = "Aimbot Toggle",
    Default = Cfg.Aimbot.Key.Name,
    Flag = "HollowPointAimbotKey",
    Mode = "Press",
})

local NightVisionKeybind = Library:RegisterKeybind({
    Name = "Night Vision Toggle",
    Default = Cfg.NV.Key.Name,
    Flag = "HollowPointNightVisionKey",
    Mode = "Press",
})
...
-- [6]  MANAGED KEYBINDS
-- ══════════════════════════════════════════════════════════════════════
-- Registered above as virtual shortcuts. RenLib's native Keybind Manager owns
-- editing, reset, config persistence, and autoload without adding tab controls.
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if keyName == AimbotKeybind:Get() then
        Aim.Toggle()
    elseif keyName == NightVisionKeybind:Get() then
        NV.Toggle()
    end
end)
...
-- ─── TAB: SETTINGS ───────────────────────────────────────────────────
local tSet  = Win:CreateTab({ Name="Settings", Emoji="⚙️" })
